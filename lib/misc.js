const { fetchJson, fetchText } = require('../function/fetcher')
 ...
 * @returns {Object}
 */
const quotesad = () => new Promise((resolve, reject) => {
    console.log('Getting random quotes...')
    fetchJson('https://videfikri.com/api/randomquotes/')
        .then((result) => resolve(result))
        .catch((err) =>  reject(err))
 ...
 * @returns {Object}
 */
 const tele = (query) => new Promise((resolve, reject) => {
    console.log(`Get YouTube search results for ${query}...`)
    fetchJson(`https://lolhuman.herokuapp.com/api/telestick?apikey=${config.lol}&url=${query}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
